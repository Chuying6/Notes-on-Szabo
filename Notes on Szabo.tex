...
\begin{equation}
  \hat{H}_{\mathrm{elec}} |\Psi\rangle = E_{\mathrm{elec}} |\Psi\rangle
\end{equation}
其解就是电子波函数
\begin{equation}
|\Psi_{\mathrm{elec}}\rangle=\Psi_{\mathrm{elec}}(\{\mathbf{r}_i\}; \{\mathbf{R}_A\})
\end{equation}
...
\begin{equation}
  ...
  \left( \frac{\zeta^3}{\pi} \right)^{1/2} e^{-\zeta |\mathbf{r}-\mathbf{R}|}
\end{equation}
$\zeta$是轨道指数,本书取1.0.但本书多数使用Gaussian轨道，因为它在积分时
更容易，Gaussian轨道形式如下
\begin{equation}
  \psi_{1s}(\mathbf{r}-\mathbf{R}) = 
...
\end{equation}
所以无穷集合 $\{|\Phi_i\rangle\}=\{|\Phi_0\rangle, |\Phi_a^r\rangle, |\Phi_{ab}^{rs}\rangle, \ldots\}$
是 $N$ 电子 Hilbert 空间的一组完备基函数，并可用于展开$N$电子波函数。由完备集
$\{|\Phi_i\rangle\}$ 产生的哈密顿算符矩阵其本征值就是基态和激发态的准确能量.
由于每个$|\Phi_i\rangle$都对应一种自旋轨道的组态(组态即生成$|\Phi_i\rangle$
...
单个粒子的自旋角动量算符是矢量算符
\begin{equation}
  \hat{\mathbf{s}}=s_x\mathbf{e}_x+s_y\mathbf{e}_y+s_z\mathbf{e}_z
\end{equation}
...
为方便，Fock算符可用$\hat{P}_{12}$重新写为
\begin{equation}
  f(1)=h(1)+\sum_b\int dx_2\chi_b^*(2)r_{12}^{-1}(1-\hat{P}_{12})\chi_b(2)
\end{equation}
...
所以变换后行列式与原行列式仅差一个相因子. 对酉矩阵而言，
\(\det(\mathbf{U})=e^{\mathrm{i}\theta}\)，不必限制为$\pm1$.
...
正如上一章讨论的, 由集合 ${\chi_i}$ 可以构造相当多的行列式. 知道了 Fock 算符的形式,
现在正好可以证明一个关于这些行列式的子集的定理. 该子集就是所有单激发行列式
$|\Phi_a^r\rangle$, 即将参考行列式 $|\Phi_0\rangle$ 中的占据轨道 $\chi_a$
替换为虚轨道 $\chi_r$ 所得的行列式. 在精确基态的多行列式表示中, 我们可能会预想,
这些单激发行列式将给出领头阶修正:
\begin{equation}
  |\Psi_{\mathrm{exact}}\rangle
  =c_0|\Phi_0\rangle+\sum_{ra}c_a^r|\Phi_a^r\rangle+\cdots
\end{equation}
若在修正中仅考虑参考行列式和单激发行列式, 那么 $c_a^r$ 可以由变分原理得到,
将 $\{\Phi_0\}, \{\Phi_a^r\}$ 基下的哈密顿矩阵对角化即可. 现在我们来研究包含单激发态的矩阵本征值问题:
\begin{equation}
  \begin{bmatrix}
    \langle\Phi_0|\hat{H}|\Phi_0\rangle & \langle\Phi_0|\hat{H}|\Phi_a^r\rangle \\
    \langle\Phi_a^r|\hat{H}|\Phi_0\rangle & \langle\Phi_a^r|\hat{H}|\Phi_a^r\rangle
...
\begin{equation}
  \langle\Phi_0|\hat{H}|\Phi_a^r\rangle
  =\langle a|h|r\rangle+\sum_{b=1}^N\langle ab||rb\rangle
\end{equation}
对于方程右边，回想Fock算符的矩阵元可以得到
\begin{equation}
  \langle a|f|r\rangle=\langle\Phi_0|\hat{H}|\Phi_a^r\rangle
\end{equation}
...
因此 $\langle\Phi_0|\hat{H}|\Phi_a^r\rangle = 0$，即单激发行列式与 Hartree-Fock 参考行列式间的矩阵元为零。
所以求解 Hartree-Fock 本征值方程等价于确保 $|\Phi_0\rangle$ 不和任何单激发行列式发生直接混合.
式\ref{eq:smatrix}中能量最低的解为
\begin{equation}
  \begin{pmatrix}
  E_0 & 0 \\
    0 & \langle\Phi_a^r|\hat{H}|\Phi_a^r\rangle
  \end{pmatrix}
  \begin{pmatrix}
    1 \\
    0
  ...
  \end{pmatrix}
\end{equation}
在这种意义下, Hartree-Fock 基态可以说是“稳定”的, 因为即使将单激发行列式混入, 结果也不能在一阶上进一步改进.
需要注意, 这并不意味着精确波函数中没有单激发成分; 单激发仍然可以通过双激发等行列式与参考态间接耦合.
Brillouin 定理表明, 单激发行列式 $|\Phi_a^r\rangle$ 不会直接与参考态即 Hartree-Fock 行列式发生作用, 
也即 $\langle\Phi_0|\hat{H}|\Phi_a^r\rangle = 0$.
\subsubsection{The Hartree-Fock Hamiltonian}
目前为止, 我们对 Hartree-Fock 近似的理解就是: Hamiltonian 是精确的,
而波函数用单 Slater 行列式来近似. 本节中, 为了第六章微扰论中的需要,
提前以另外一种等价但不同的视角研究 Hartree-Fock 理论, 此时我们主要关注的是 Hamiltonian 本身.

是否存在某个近似的 N-电子 Hamiltonian,其本征值方程和我们之前精确求解过的完全相同?或者说,
答案是存在. Hartree-Fock Hamiltonian 定义如下:
  \begin{equation}
    \hat{H}_0 =\sum_{i=1}^N\hat{f}(i)
  \end{equation}
上式对应的本征值不是 Hartree-Fock 能量 $E_0$, 而是轨道能量之和 $\sum_a\varepsilon_a$.
实际我们可以证明, 由 Fock 算符本征函数 $\{\chi_p\}$ 组成的任意行列式都是 $\hat{H}_0$ 的本征函数,
其本征值等于该行列式中自旋轨道能量之和. 因此, $|\Phi_0\rangle$ 是 $\hat{H}_0$ 的一个本征函数,
其本征值为 $\sum_{a=1}^N\varepsilon_a$.
这个本征值当然不等于 $E_0$ 本身. 两者的关系为:
\begin{equation}
  E_0 = \langle\Phi_0|\hat{H}|\Phi_0\rangle
  = \sum_{a=1}^N\varepsilon_a
  - \frac12\sum_{a=1}^N\sum_{b=1}^N\langle ab||ab\rangle
\end{equation}
\subsubsection{Restricted Closed-Shell Hartree--Fock: The Roothaan Equations}
...
闭壳层限制性基态表示为
\begin{equation}
  |\Psi_0\rangle=|\chi_1\bar{\chi}_1\chi_2\bar{\chi}_2\cdots\chi_{N/2}\bar{\chi}_{N/2}\rangle=|\psi_1\bar{\psi}_1\psi_2\bar{\psi}_2\cdots\psi_{\frac{N}{2}}\bar{\psi}_{\frac{N}{2}}\rangle
\end{equation}
...
\begin{equation}
  ...
  &\qquad +\Big[\sum_{c}\int d\omega_1d\mathbf{x}_2\,
     \alpha^*(\omega_1)\chi_c^*(\mathbf{x}_2)r_{12}^{-1}(1-\hat{P}_{12})
...
Fock 矩阵 $F$ 是 Fock 算符在基组 ${\phi_{\mu}}$ 下的矩阵表示
\begin{equation}
  \begin{aligned}
      F_{\mu\nu}&=\int d\tau_1\phi_{\mu}^*(1)f(1)\phi_{\nu}(1) \\
...
\begin{equation}
 (\mathbf{X}^{\dagger}\mathbf{F}\mathbf{X}\mathbf{C})=(\mathbf{X}^{\dagger}\mathbf{S}\mathbf{X})\mathbf{C}\varepsilon
\end{equation}
定义新矩阵$\mathbf{F}'$为
\begin{equation}
...
\subsection{Model Calculations on \texorpdfstring{$\mathrm{H}_2$}{H2} and \texorpdfstring{$\mathrm{HeH}^{+}$}{HeH+}}
...
\subsubsection{STO-3G \texorpdfstring{$\mathrm{H}_2$}{H2}}
本小节我们进行基态 $\mathrm{H}_2$的限制性闭壳层计算.

我们使用和$\mathrm{H}_2$ 极小基一样的坐标，核间距$\mathbf{R}_12=1.4 a.u.$ 并选用标准的STO-3G基组。
下一步进行基组${\phi_\mu }$ 形成的积分，即 $S_{\mu \nu}$,$H^{core}_{\mu\nu}$ 和$(\mu\nu|\lambda\sigma))$
...
\begin{equation}
  ...
  =\mathbf{T}+\mathbf{V}^{1}+\mathbf{V}^{2}
  =
  \begin{pmatrix}
    -1.1204 & -0.9584 \\
    -0.9584 & -1.1204
...
\begin{equation}
  \phi_{1s}(\mathbf{r})=\pi^{-1/2}\exp(-r),
\end{equation}
则有 $T_{11}=0.5$ 以及 $V^{1}_{11}=-1.0$，从而氢原子的单电子能量为 $-0.5$~a.u.
在此处的 STO-3G 基函数中，轨道比精确氢原子 $1s$ 轨道更收缩，因此电子更靠近核：
$V^{1}_{11}=-1.2266$ 更负；相应地，为避免电子坍缩到核上，动能也增大为 $T_{11}=0.7600$。

动能矩阵 $\mathbf{T}$ 与核吸引矩阵 $\mathbf{V}^{\mathrm{nuc}}=\mathbf{V}^{1}+\mathbf{V}^{2}$ 的非对角元
没有简单的经典解释。
当核间距 $R$ 很大时，原子轨道之间的重叠消失，这些非对角元也趋于零。

先求解核心 Hamiltonian 的广义本征值方程
\begin{equation}
  \mathbf{H}^{\mathrm{core}}\mathbf{C}
  =\mathbf{S}\mathbf{C}\boldsymbol{\varepsilon},
\end{equation}
即可得到极小基 $\mathrm{H}_2^+$ 的分子轨道和轨道能量。随后，对中性 $\mathrm{H}_2$ 还需构造
依赖电子密度的 Fock 矩阵，并求解 Roothaan 方程。

由于该模型具有同核双原子分子的对称性，其解可直接由对称性确定。正则分子轨道
(canonical molecular orbitals) 按分子点群的不可约表示变换；对于同核双原子分子，
轨道可标记为 $\sigma_g$、$\sigma_u$、$\pi_g$、$\pi_u$ 等。在极小基模型中只有两个分子轨道：
能量较低的占据轨道是具有 $\sigma_g$ 对称性的成键轨道
\begin{equation}
  \psi_1=[2(1+S_{12})]^{-1/2}(\phi_1+\phi_2),
\end{equation}
而虚分子轨道是具有 $\sigma_u$ 对称性的反键轨道
\begin{equation}
...
对于基态闭壳层 $\mathrm{H}_2$，两个电子均占据 $\psi_1$，故密度矩阵为
\begin{equation}
  \mathbf{P}=
  \begin{pmatrix}
    (1+S_{12})^{-1} & (1+S_{12})^{-1} \\
  ...
  \end{pmatrix}.
\end{equation}
在这里的限制性闭壳层模型中，若 SCF 迭代的初始密度矩阵与上述结果不同，
只要迭代保持分子对称性，计算就会收敛到这一由对称性确定的解。

在正则分子轨道基下，变换后的 Fock 矩阵元为
\begin{equation}
  f_{ij}=\langle\psi_i|\hat{f}|\psi_j\rangle.
\end{equation}
根据正则 Hartree--Fock 方程，该矩阵是对角矩阵，其对角元即为轨道能量。
闭壳层体系的轨道能量可写为
\begin{equation}
  \varepsilon_i=h_{ii}+\sum_{b}^{N/2}\left(2J_{ib}-K_{ib}\right).
  \label{eq:rhf-orbital-energy-minimal-h2}
\end{equation}
在此处的极小基模型中，只有轨道 $1$ 被占据，因而
\begin{align}
  \varepsilon_1&=h_{11}+J_{11}=-0.5782~\text{a.u.}, \\
  \varepsilon_2&=h_{22}+2J_{12}-K_{12}=+0.6703~\text{a.u.}.
\end{align}
注意 $\varepsilon_2\neq h_{22}+J_{22}$。这是因为轨道 $2$ 是虚轨道；它描述的是在冻结
其余轨道的近似下，将一个电子加入该轨道后 $(N+1)$ 电子体系中该电子的能量，
这正是 Koopmans 定理中的含义。

基态的电子能量为
\begin{equation}
  E_0=2h_{11}+J_{11}=-1.8310~\text{a.u.}
\end{equation}
加上核--核排斥能后，分子的总能量为
\begin{equation}
  E_{\mathrm{tot}}=E_0+\frac{1}{R}=-1.1167~\text{a.u.}
\end{equation}
由于在该 STO-3G 基组下，氢原子能量为 $-0.4666$~a.u.，预测的解离能为
\begin{equation}
  D_e=2(-0.4666)-(-1.1167)=0.1835~\text{a.u.}=4.99~\text{eV}.
\end{equation}
这一数值与实验值 $D_e\approx4.75$~eV 相当接近。

不过，要完整讨论解离，还必须考察不同核间距下的总能量，也就是势能曲线
(potential-energy curve)。在每个 $R$ 上重复上述计算，即可得到 $E_{\mathrm{tot}}(R)$。
极小基限制性闭壳层 Hartree--Fock 在平衡键长附近可以给出合理结果，但在 $R\to\infty$ 时
不能正确描述两个开壳层氢原子的解离极限：限制性解会强制两个电子继续占据同一个空间轨道。
之后讨论非限制性 Hartree--Fock (unrestricted Hartree--Fock, UHF) 时，会进一步分析这一解离问题。
计算得到的平衡构型，就是 $E_{\mathrm{tot}}$ 关于核坐标取最小值时的构型。

\subsubsection{STO-3G \texorpdfstring{$\mathrm{HeH}^{+}$}{HeH+}: SCF Iteration}

极小基 $\mathrm{H}_2$ 的分子轨道可由同核双原子分子的对称性直接确定，因而不能充分展示
自洽场迭代。$\mathrm{HeH}^{+}$ 是异核双原子分子，两个原子轨道不等价，分子轨道系数
不能预先由对称性给出；它因此是理解 Roothaan 方程迭代求解的最小模型。

在平衡核间距 $R=1.4632$~a.u. 处，以 He 上的 $1s$ 基函数 $\phi_1$ 和 H 上的 $1s$
基函数 $\phi_2$ 构成极小 STO-3G 基组。指数通常取 $\zeta_{\mathrm H}=1.24$；He 的
``原子最优''指数为 $\zeta_{\mathrm{He}}=27/16=1.6875$。这里需要区分：基函数指数
(exponent) 是非线性参数，而 SCF 中更新的 MO 系数和密度矩阵是线性参数。

\paragraph{SCF 的核心闭环}
取 $\mathbf{P}^{(0)}=\mathbf{0}$ 时，电子--电子项 $\mathbf{G}$ 为零，故初始 Fock 矩阵为
\begin{equation}
  \mathbf{F}^{(0)}=\mathbf{H}^{\mathrm{core}}.
\end{equation}
随后不断重复
\begin{equation}
  \mathbf{P}^{(n)}
  \longrightarrow \mathbf{G}[\mathbf{P}^{(n)}]
  \longrightarrow \mathbf{F}^{(n)}=\mathbf{H}^{\mathrm{core}}+\mathbf{G}[\mathbf{P}^{(n)}]
  \longrightarrow \mathbf{F}^{(n)}\mathbf{C}^{(n+1)}
  =\mathbf{S}\mathbf{C}^{(n+1)}\boldsymbol{\varepsilon}^{(n+1)}
  \longrightarrow \mathbf{P}^{(n+1)},
\end{equation}
直至密度矩阵或能量不再改变，即收敛达到自洽。每一步可用与该 Fock
矩阵相配套的密度矩阵计算电子能量
\begin{equation}
  E_0=\frac{1}{2}\sum_{\mu\nu}P_{\nu\mu}
  \left(H^{\mathrm{core}}_{\mu\nu}+F_{\mu\nu}\right).
\end{equation}
这个例子中，忽略电子--电子排斥的初猜会使电子过度偏向核电荷更大的 He；加入
Coulomb--exchange 平均场后，电子密度部分向 H 端重新分布，能量单调收敛。
...
\begin{equation}
  ...
  \end{pmatrix}\ \text{a.u.}
\end{equation}
占据轨道中 $\phi_1$ 的系数较大，说明电子密度主要位于 He 附近；但它仍含有 H 的同相
成分，故为成键轨道 (bonding orbital)。相应的反键虚轨道中两个系数异号。

总能量需加入核--核排斥项 $Z_{\mathrm{He}}Z_{\mathrm H}/R=2/R$：
\begin{equation}
  E_{\mathrm{tot}}=E_0+\frac{2}{R}=-2.860662~\text{a.u.}
\end{equation}
该极小基模型正确预测了解离产物为闭壳层的 $\mathrm{He}+\mathrm{H}^+$，而不是
$\mathrm{He}^{+}+\mathrm H$。这也解释了它在大 $R$ 时比限制性闭壳层 $\mathrm H_2$
表现更合理：其正确解离极限本身就是闭壳层。不过，预测的解离能
$D_e=0.2168$~a.u. ($5.90$~eV) 高于精确值约 $2.04$~eV；主要原因是同一套固定的 He
指数更适合分子而非孤立 He 原子，反映的是基组与指数的局限，而非 SCF 迭代本身失效。

\subsection{多原子基组 (Polyatomic Basis Sets)}

本节比较四类常用基组：STO-3G、4-31G、6-31G* 与 6-31G**。这些基组在 Gaussian 系列
程序中广泛使用，其名称中的数字表示收缩方式。

基组变大通常会降低变分能量，但计算量也会显著增加；在 Hartree--Fock 计算中，最耗时的
双电子积分及其变换通常随基函数数 $K$ 快速增长，常以 $K^4$ 作为量级估计。

\subsubsection{收缩型高斯函数 (Contracted Gaussian Functions)}

分子轨道在原子轨道基组中的线性展开为
\begin{equation}
  \psi_i(\mathbf r)=\sum_{\mu=1}^{K}C_{\mu i}\phi_\mu(\mathbf r).
  \label{eq:mo-lcao-expansion}
\end{equation}
选择基函数时有两难的问题：Slater 型函数 (STO) 的形状更接近真实原子轨道，
但 Gaussian 型函数 (GTO) 的积分更快。收缩型高斯函数 (CGF) 用固定线性组合把二者结合：
\begin{equation}
  \phi_\mu^{\mathrm{CGF}}(\mathbf r-\mathbf R_A)
  =\sum_{p=1}^{L}d_{p\mu}\,
  g_p(\alpha_{p\mu},\mathbf r-\mathbf R_A).
  \label{eq:contracted-gaussian}
\end{equation}
其中 $g_p$ 是归一化的原初高斯函数 (primitive Gaussian function)，$\alpha_{p\mu}$
是指数，$d_{p\mu}$ 是收缩系数，$L$ 是收缩长度 (contraction length)。\textbf{一个 CGF
才算一个基函数}；其中包含的多个原初高斯函数不会使 MO 展开的维数增加。

这一区分十分重要：先在原子计算中用较大的未收缩基组优化指数和系数，再把若干原初函数
固定收缩成较少的 CGF，便能以较低代价保留大部分原子轨道形状。收缩主要减少计算成本，
但也会牺牲部分适应不同分子环境的变分灵活性。

对于同一 $sp$ 壳层，$s$ 与三个 $p$ 分量通常共享一套指数。这样既使它们具有相同的径向
行为，又可复用径向积分；但要记住一个 $p$ 壳层对应 $p_x,p_y,p_z$ 三个基函数。

\subsubsection{极小基组：STO-3G (Minimal Basis Sets)}

极小基组 (minimal basis set) 的原则是：每个占据的原子价层轨道至少给一个径向基函数。对
H、He 为 $1$ 个 $s$ 函数；对第二周期 Li--Ne，通常为 $1s+2s+2p_x+2p_y+2p_z$，共 $5$
个函数。后者看似多给了未占据的 $2p$ 轨道，但这是为了完整表示 $sp$ 壳层，并保持原子的
角向自由度。

STO-$L$G 表示用 $L$ 个原初 GTO 的固定收缩近似一个 STO；例如 STO-3G 的每个收缩函数
由三个原初高斯函数组成。原初指数大致随有效核电荷平方改变，$\alpha\propto\zeta^2$。
因此同一原子在不同化学环境中的最优轨道指数 (orbital exponent) 并不完全相同：
\begin{itemize}
  \item 为孤立原子优化的指数，最适合孤立原子；
  \item 针对每一个分子重新优化指数，最灵活，但会引入昂贵的非线性优化；
  \item 标准基组采用折衷指数，使一套参数能服务一批分子。
\end{itemize}
例如前面 $\mathrm{H_2}$ 的 STO-3G 氢 $1s$ 指数取 $\zeta=1.24$，只是这种折衷参数，并非
所有键长和所有氢化合物的严格最优值。

\subsubsection{双 zeta 与劈裂价层基组 (Double-Zeta and Split-Valence Basis Sets)}

单一径向函数很难同时描述原子核附近较紧密、以及成键或极化后较扩展的电子密度。双
$\zeta$ 基组 (double-zeta basis set) 为同一个原子轨道提供两套不同径向尺度的函数；MO
系数在线性变分中自行混合它们，从而获得径向弹性，而不必在每个 SCF 循环中重新优化非线性
指数。

4-31G 是劈裂价层基组 (split-valence basis set)，不是所有壳层都严格的双 $\zeta$：内层
(core) 仍用一组紧密函数，价层 (valence) 则劈裂成内、外两组。命名 ``4-31'' 的含义是：
内壳层收缩函数由 $4$ 个原初高斯组成；价层被写成一组 $3$ 原初函数的收缩函数加上一组
单原初函数。对 C 等第二周期原子，计数为
\[
  1\;(\text{core }s)+4\;(\text{inner }sp)+4\;(\text{outer }sp)=9;
\]
对 H 则有两组 $s$ 函数，故为 $2$。\textbf{这里的 4、3、1 描述收缩方式，不直接等于
最终基函数数目。}

\subsubsection{极化函数 (Polarization Functions)}

即使价层已有两种径向尺度，若每个原子仍只保留原子态固有的角动量，电子云也难以朝键轴或
邻近电荷方向偏移。极化函数 (polarization functions) 就是在原子上加入较高角动量函数，
以提供这种角向变形。例如氢原子在外电场或分子中非球形电场下，波函数需要将原来的 $1s$
成分与 $p$ 成分混合；这不是说孤立氢原子本身有被占据的 $p$ 轨道，而是分子环境打破了球
对称性。

6-31G* 在第二周期重原子（如 C、N、O、F）上加入一组 $d$ 极化函数；6-31G** 再在 H
上加入一组 $p$ 极化函数。按本书的笛卡尔表示，一组 $d$ 函数为
\[
  d_{xx},d_{yy},d_{zz},d_{xy},d_{xz},d_{yz},
\]
共 $6$ 个；它可分解为 $5$ 个纯 $d$ 函数与一个 $s$ 型组合。不同程序也可能采用 $5$ 个
球谐 $d$ 函数，故比较基函数数目时必须先确认约定。6-31G* 与 6-31G** 通常比极小基或仅
劈裂价层基组给出更好的定量结果，但高精度还可能需要更多 $\zeta$、多套极化函数及弥散函数
(diffuse functions)。

\paragraph{练习 3.31：苯的基函数数目}
计算的是\textbf{收缩基函数}数目，而不是原初高斯函数数目。苯为 $\mathrm{C_6H_6}$；本书采用
笛卡尔 $d$ 函数，故一个 $d$ 壳层含六个函数。
\begin{align}
  N_{\mathrm{STO\text{-}3G}}
  &=6\times 5_{\mathrm C}+6\times 1_{\mathrm H}=36,\\
  N_{\mathrm{4\text{-}31G}}
  &=6\times 9_{\mathrm C}+6\times 2_{\mathrm H}=66,\\
  N_{\mathrm{6\text{-}31G^*}}
  &=6\times(9+6)_{\mathrm C}+6\times 2_{\mathrm H}=102,\\
  N_{\mathrm{6\text{-}31G^{**}}}
  &=6\times(9+6)_{\mathrm C}+6\times(2+3)_{\mathrm H}=120.
\end{align}
STO-3G 中 C 为 $1s+2s+2p_x+2p_y+2p_z=5$，H 为 $1$；4-31G 中 C 为
$1+4+4=9$，H 为 $2$。6-31G* 给每个 C 加 $6$ 个笛卡尔 $d$ 函数；6-31G** 再给每个 H
加 $3$ 个 $p$ 函数。若采用五个球谐 $d$ 函数，后两项会变为 $96$ 与 $114$。

\subsection{闭壳层分子计算示例 (Closed-Shell Molecular Calculations)}

本节用几个闭壳层分子说明：基组变大时总能量会下降，但化学上更常比较能量差、结构和分子性质。

\subsubsection{总能量 (Total Energy)}

总能量由电子能量和核--核排斥组成：
\[
  E_{\mathrm{tot}}=E_0+\sum_{A<B}\frac{Z_AZ_B}{R_{AB}}.
\]
固定构型下，基组越大，HF 能量越低并趋近 HF 极限；但 HF 极限仍缺少电子相关
(electron correlation)。反应能使用
\(\Delta E=\sum E_{\mathrm{products}}-\sum E_{\mathrm{reactants}}\)。它不再有变分上界，
但使用均衡基组时基组误差常可部分抵消。\textbf{绝对能量主要看收敛；化学反应主要看能量差。}

对 $\mathrm{H_2}$，6-31G* 不增加函数，故等价于 4-31G；对不含 H 的 $\mathrm{CO}$、
$\mathrm{N_2}$，6-31G** 等价于 6-31G*。

\subsubsection{电离势 (Ionization Potentials)}

Koopmans 定理给出
\[
  \mathrm{IP}\approx-\varepsilon_{\mathrm{HOMO}},
\]
其中 HOMO (highest occupied molecular orbital) 是最高占据轨道。$\mathrm{H_2}$ 的占据轨道在较大
基组下约为 $-0.595\ E_h$，所以预测电离势为 $+0.595\ E_h$，接近实验值 $0.584\ E_h$。

表中的电离势是\textbf{垂直电离势} (vertical IP)：电离前后核位置不变；绝热电离势允许离子
和中性分子各自优化结构。$\mathrm{H_2}$ 的好结果来自相关效应和离子轨道弛豫 (orbital relaxation)
的误差抵消。Koopmans 适合初步归属占据轨道，不能可靠预测电子亲和能；$\mathrm{N_2}$ 就是它
可能给出错误定性轨道排序的反例。

\subsubsection{平衡几何构型 (Equilibrium Geometries)}

在 Born--Oppenheimer 近似下，$E_{\mathrm{tot}}(\mathbf R)$ 定义势能面 (potential energy
surface, PES)。平衡结构是势能面的极小点：梯度为零，Hessian 为正定；鞍点则对应过渡态。

几何优化 (geometry optimization) 是对核坐标的非线性最小化，实际计算通常借助能量梯度。
RHF 在平衡点附近可预测合理结构，但不能可靠描述开壳层解离区。HF 常将键长预测得略短；
极小基的误差更大。对键角，$d$ 极化函数很重要，例如只用 $s,p$ 基组可能错误把 $\mathrm{NH_3}$
预测为平面结构。

\subsubsection{布居分析与偶极矩}
...
\]
偶极矩 (dipole moment) 是电子贡献与核贡献的和：
\[
  \boldsymbol\mu=-\sum_{\mu\nu}P_{\mu\nu}(\nu|\mathbf r|\mu)
  +\sum_AZ_A\mathbf R_A.
\]

Mulliken 和 L\"owdin 布居分析 (population analysis) 都把电子密度按某种规则分给各原子，
从而得到原子电荷。\textbf{原子电荷不是唯一的可观测量：它依赖分区方案和基组。}因此可用同一
方法、同一基组比较电荷变化趋势，但不应把单一 Mulliken 电荷当作绝对事实。
